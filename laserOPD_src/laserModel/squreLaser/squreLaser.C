...
namespace Foam
{
    namespace radiation
    {
        defineTypeNameAndDebug(squreLaser, 0);
        addToRunTimeSelectionTable(laserModel, squreLaser, dictionary);
    }
}
...
Foam::radiation::squreLaser::squreLaser
(
...
void Foam::radiation::squreLaser::createCloud(
        ...
        for (label xi = 0; xi < nLength_; xi++)
        {
            ...
            for (label yi = 0; yi < nLength_; yi++)
            {
                ...
                if (cellI != -1)
                {
                    ...
                    laserParticle* pPtr =
                        new laserParticle(mesh_, xy, p0, p1, cellI, dAi);
